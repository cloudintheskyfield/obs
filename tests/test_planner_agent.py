...
def test_planner_filters_browser_description_out_of_test_commands() -> None:
    ...
    assert any(command.startswith("python -c ") and "index.html" in command for command in command_texts)
    ...
    contract = _normalize_plan_contract(
        {
            "goal": "创建 index.html",
            "allowed_files": ["index.html"],
            "test_commands": [{"name": "syntax", "cmd": "node --check game.js", "timeout_sec": 30}],
        },
        "创建 index.html",
        [],
    )
