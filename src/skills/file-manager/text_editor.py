...
def _build_readonly_roots() -> List[Path]:
    ...
    if skills_env:
        ...
        if p.exists():
            roots.append(p)
    for candidate in [Path("/app/src/skills")]:
        if candidate.exists():
...
class TextEditorSkill(BaseSkill):
    ...
    def __init__(self, work_dir: str = "workspace"):
        ...
        self.add_parameter("command", "str",
            "Command: view, create, write, append, str_replace, insert, delete, find, undo_edit. "
            "For large files (HTML/CSS/JS games or any file >4KB), never send the full source as one argument. "
            "Use write for a tiny skeleton, then append real chunks under 1500 chars each.", True)
